...
function getAuthHeaders(method, requestPath, body = "") {
  ...
  // The DEX aggregator (quote/swap) endpoints additionally require a project ID —
  // found on your project's page in the Developer Portal, next to your API keys.
  // Market data endpoints (price-info) work fine without it.
...
// Real, live, read-only swap pricing from OKX's DEX aggregator — no funds move, nothing
// is approved or executed. This only works meaningfully on MAINNET, since that's where
// real liquidity actually exists (testnet has no organic trading pairs to route through).
export async function getSwapQuote({ fromTokenAddress, toTokenAddress, amount, chainIndex = "196" }) {
  const params = new URLSearchParams({ chainIndex, amount, fromTokenAddress, toTokenAddress });
  const requestPath = "/api/v6/dex/aggregator/quote";
  const queryString = `?${params.toString()}`;
  const headers = getAuthHeaders("GET", requestPath + queryString, "");

  if (!headers) {
    console.log("[Sentinel] OKX credentials not set — cannot fetch swap quote");
  ...
  try {
    ...
    if (!res.ok) {
      const errText = await res.text();
      console.log("[Sentinel] OKX Quote API error:", res.status, errText);
      return { available: false };
    }

    const json = await res.json();
    if (json.code !== "0" || !json.data?.[0]) {
      console.log("[Sentinel] OKX Quote API returned error/no route:", json.code, json.msg);
      return { available: false };
    }
    ...
    return {
      ...
      toTokenAmount: q.toTokenAmount ?? q.originToTokenAmount,
      fromTokenDecimals: Number(q.fromToken?.decimal ?? 18),
      toTokenDecimals: Number(q.toToken?.decimal ?? 18),
      estimateGasFee: q.estimateGasFee,
      priceImpactPercentage: q.priceImpactPercentage,
    };
  } catch (e) {
    console.log("[Sentinel] OKX Quote API fetch threw:", e.message);
    return { available: false };
  }
}
